...
export default function RegisterPage() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok) {
        const msg = typeof data === "string" && data ? data : data?.error || data?.message || "Registration failed";
        throw new Error(msg);
      }
              ...
              <div className="rounded-xl border border-[#29d3ff]/40 bg-white/5 p-5 shadow-inner">
                <form onSubmit={onSubmit} className="space-y-3">
                  <div className="grid grid-cols-2 gap-3">
                    <input
                      className="rounded-md border border-white/10 bg-white/10 px-3 py-2 outline-none placeholder:text-white/70 focus:ring-2 focus:ring-[#29d3ff]"
